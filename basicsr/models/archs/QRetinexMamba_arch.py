...
from .qretinex.quaternion import hamilton_product
...
# ---------------
# If your original Mamba code references these:
from .SS2D_arch import SS2D     # or wherever your SS2D is
from .IFA_arch import IFA       # or your IFA module
# ---------------

# For Hamilton product:
from .qretinex.quaternion import hamilton_product


# --------------------------------------------------------------------
#  Simple gelu / truncated normal helpers
# --------------------------------------------------------------------
class GELU(nn.Module):
    def forward(self, x):
        return F.gelu(x)
...
def trunc_normal_(tensor, mean=0., std=1.):
    """
    Example truncated normal initialization (same as your original Mamba).
    Implement or import your own as needed.
    """
    # You can keep your own version, or a no-op, etc.
    # Just a placeholder:
    with torch.no_grad():
...
#    (PreNorm, FeedForward, IGAB but simplified to not require a second illu_fea)
# --------------------------------------------------------------------
class PreNorm(nn.Module):
...
class FeedForward(nn.Module):
    """
    The same feed-forward block you had, but unchanged.
    Just ensure it can handle 'dim=4' if needed.
    """
    def __init__(self, dim, mult=4):
        super().__init__()
        self.net = nn.Sequential(
...
class IGAB(nn.Module):
    """
    Interleaved Group Attention Block (as in your Mamba code).
    Adapted so that we no longer pass a second 'illu_fea' argument.
    We'll just do self-attention on x.
    """
    def __init__(self, dim, dim_head=64, heads=8, num_blocks=2, d_state=16):
        super().__init__()
        self.blocks = nn.ModuleList([])
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                # Instead of IFA with illu_fea, we can do IFA with the same input x
                # or keep your same IFA signature if it is purely self-attn
    ...
    def forward(self, x):
        # x: [b, c, h, w]. We'll do the same style as before
        for (trans, ss2d, ff) in self.blocks:
            # trans can treat x as [b,c,h,w], or you might internally do x->[b,h,w,c]
            y = trans(x)                 # e.g. your IFA might do x->some attn
            y = y.permute(0, 2, 3, 1)    # [b,h,w,c]
            # SS2D is also expecting [b,h,w,c]? or [b,c,h,w]? 
            # If your SS2D wants [b,h,w,c], feed it directly:
            y2 = ss2d(y)                 # returns same shape [b,h,w,c]
            # then do a residual add
            x = x.permute(0, 2, 3, 1)
            x = y2 + x                  # residual
            # feed-forward block
            x = ff(x) + x
            # revert to [b,c,h,w] for next iteration
            x = x.permute(0, 3, 1, 2)
        return x

# --------------------------------------------------------------------
#  The "Denoiser" (or "QuatNet") that handles a 4-channel quaternion
# --------------------------------------------------------------------
class Denoiser(nn.Module):
    """
    Mamba-style encoder-decoder, but now we default to in_dim=4, out_dim=4
    so it can process a quaternion (Q1 or Q2).
    """
    def __init__(self, in_dim=4, out_dim=4, dim=31, level=2, num_blocks=[2, 4, 4], d_state=16):
        super(Denoiser, self).__init__()
        self.dim = dim
        self.level = level

        # Input embedding (was called 'embedding' before)
    ...
    def forward(self, x):
        """
        x: [B,4,H,W] quaternion data (either Q1 or Q2)
        """
        ...
        # Decoding
        for i, (FeaUpSample, Fution, IGABlock) in enumerate(self.decoder_layers):
            fea = FeaUpSample(fea)
            skip = fea_encoder[self.level - 1 - i]
            fea  = torch.cat([fea, skip], dim=1)
            fea  = Fution(fea)
            fea  = IGABlock(fea)

        out = self.mapping(fea) + x  # optional residual
        return out


# --------------------------------------------------------------------
#  Single stage “RetinexMambaQuat_Single_Stage”
#    Takes Q1, Q2 => processes => hamilton_product => final
# --------------------------------------------------------------------
class RetinexMambaQuat_Single_Stage(nn.Module):
    """
    A single stage that takes in the 4-channel reflectance Q1
    and 4-channel illumination Q2, processes each, then recombines.
    """
    def __init__(self, n_feat=31, level=2, num_blocks=[2,4,4], d_state=16):
        super().__init__()
        # Two "denoisers" (or "enhancers") for reflectance/illumination
        self.reflect_net = Denoiser(in_dim=4, out_dim=4,
                                    dim=n_feat, level=level,
                                    num_blocks=num_blocks,
                                    d_state=d_state)
        self.illum_net   = Denoiser(in_dim=4, out_dim=4,
                                    dim=n_feat, level=level,
                                    num_blocks=num_blocks,
                                    d_state=d_state)
    ...
    def forward(self, Q1, Q2):
        """
        Q1, Q2: [B,4,H,W]
        Returns either final quaternion or the last 3 channels.
        """
        Q1_enh = self.reflect_net(Q1)   # [B,4,H,W]
        Q2_enh = self.illum_net(Q2)     # [B,4,H,W]

        Q_out  = hamilton_product(Q1_enh, Q2_enh)  # [B,4,H,W]

        # Option: interpret channels 1..3 as final RGB
        final_im = Q_out[:, 1:, :, :]   # [B,3,H,W]
        return final_im


# --------------------------------------------------------------------
#  Multi-stage “RetinexMambaQuat”
#    Repeats multiple single-stage modules, each refining Q1,Q2
# --------------------------------------------------------------------
class RetinexMambaQuat(nn.Module):
    """
    Example multi-stage that repeatedly processes (Q1, Q2).
    On each stage, you might feed the output Q1-> next stage Q1,
    or do some other logic if you want accumulative updates, etc.
    """
    def __init__(self, n_feat=31, stage=3, num_blocks=[2,4,4], d_state=16):
        super().__init__()
        # We have "stage" many single-stage modules
        # If you want each stage to share weights, you can do so,
        # or create them independently.
        self.stages = nn.ModuleList([
            RetinexMambaQuat_Single_Stage(
                n_feat=n_feat,
                level=2,
                num_blocks=num_blocks,
                d_state=d_state
    ...
    def forward(self, Q1, Q2):
        ...
        for i, stage_module in enumerate(self.stages):
            # Get the stage's result on Q1,Q2
            out = stage_module(Q1, Q2)  
            # out is the final [B,3,H,W], or you can keep [B,4,H,W]
            # if you want to track the next Q1/Q2.
            #
            # In some Retinex designs, you might want to also *update*
            # Q1 or Q2. For instance, if each stage modifies Q1/Q2.
            # That logic is up to you. 
            #
            # For demonstration, let's just keep overwriting Q1, Q2
            # with the stage's outputs (pretending we still have 4-ch).
            # Or you can decide to do something else.
            pass

        return out  # final result from last stage
